...
for($c=0;$c<10-2;++$c) {
printf "int* aa$c = (int*)malloc(2048*sizeof(int));\n";
printf "if (!aa$c) exit(__LINE__);\n";
printf "int* bb$c = (int*)malloc(2048*sizeof(int));\n";
...
for($c=0;$c<10-2;++$c) {
printf "cl_mem bufa$c = clCreateBuffer(ctx,CL_MEM_USE_HOST_PTR,2048*sizeof(int),aa$c,&err);\n";
printf "if (err) exit(__LINE__);\n";
printf "cl_mem bufb$c = clCreateBuffer(ctx,CL_MEM_USE_HOST_PTR,2048*sizeof(int),bb$c,&err);\n";
printf "if (err) exit(__LINE__);\n";
}

printf "size_t file_sz;\n";
printf "void* pfile;\n";
printf "__mapfile(\"test_fa_wb.cl\",file_sz,pfile);\n";
...
for($j=0;$j<$b;++$j) {
printf "if (clEnqueueReadBuffer(cmdq,bufb$j,CL_TRUE,0,128*sizeof(int),bb$j,0,0,&ev[1+$j])) \n";
printf "exit(__LINE__);\n";
}
