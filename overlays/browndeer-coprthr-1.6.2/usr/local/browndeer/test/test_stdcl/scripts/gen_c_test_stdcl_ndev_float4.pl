...
printf "int ndev = devnum + 1;\n";
printf "if (ndev > clgetndev(cp)) exit(__LINE__);\n";
...
printf "if (!bb".$c."[n]) exit(__LINE__);\n";
}
printf "}\n";
...
printf "for(n=0;n<ndev;n++) {\n";
for($i=0;$i<$a;++$i) {
printf "if (!clmsync(cp,n,aa".$i."[n],CL_MEM_DEVICE|CL_EVENT_NOWAIT))\n";
...
printf "for(n=0;n<ndev;n++) {\n";
for($j=0;$j<$b;++$j) {
printf "if (!clmsync(cp,n,bb".$j."[n],CL_MEM_HOST|CL_EVENT_NOWAIT))\n";
printf "exit(__LINE__);\n";
}
printf "}\n";

printf "for(n=0;n<ndev;n++) {\n";
printf "clwait(cp,n,CL_KERNEL_EVENT|CL_MEM_EVENT);\n";
printf "}\n";
...
printf "sum_correct += (j+1.1)*( (ns*(ns-1)*1.1)/2 + (1+13.1)*i*ns + 0.1*ns + ns*0.4575);\n";

printf "sum = 0;\n";
printf "for(n=0;n<ndev;n++) {\n";
for($j=0;$j<$b;++$j) {
printf "for(i=0;i<size;i++) sum += bb".$j."[n][i];\n";
}
printf "}\n";
